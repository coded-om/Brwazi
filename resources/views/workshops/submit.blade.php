<x-layout>
    <section class="max-w-3xl mx-auto px-4 py-12">
        <div class="mb-10 text-center space-y-3">
            ...
            <p class="text-slate-500 text-sm">يمكن للأعضاء الموثقين اقتراح ورشات، وسيقوم فريق الإدارة بمراجعتها
                والموافقة عليها قبل ظهورها للجميع.</p>
        </div>
        ...
        @if(!auth()->user() || !auth()->user()->isVerified())
            <div class="rounded-3xl bg-white p-8 shadow-sm ring-1 ring-rose-200/50 border border-rose-100 text-center">
                ...
                <a href="{{ route('user.dashboard') }}"
                    class="inline-flex items-center gap-2 rounded-full bg-slate-800 text-white px-6 py-3 text-sm font-semibold hover:bg-slate-900">العودة
                    للوحة التحكم</a>
            </div>
        @else
            <form action="{{ route('workshops.submit.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                <div class="rounded-3xl bg-white shadow-sm ring-1 ring-slate-100 p-8 space-y-8">
                    <div class="grid gap-6 md:grid-cols-2">
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <input name="title" value="{{ old('title') }}" required
                                ...
                                placeholder="مثال: أساسيات الرسم الزيتي" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="presenter_name"
                                value="{{ old('presenter_name', auth()->user()->full_name ?? '') }}" required
                                class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100" />
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="art_type" value="{{ old('art_type') }}"
                                class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100" />
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <input name="location" value="{{ old('location') }}"
                                class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                placeholder="مثال: جدة - مركز الفن الحديث أو عبر الزوم" />
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="short_description" rows="4"
                            class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                            placeholder="لمحة عن محتوى الورشة وهدفها">{{ old('short_description') }}</textarea>
                    ...
                    <div class="space-y-6">
                        <div class="grid gap-6 md:grid-cols-2">
                            <div class="space-y-2">
                                ...
                                <input name="external_apply_url" value="{{ old('external_apply_url') }}"
                                    class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                    placeholder="https://..." />
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="presenter_bio" rows="3"
                                class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                                placeholder="خبرة المقدم ومجال تخصصه">{{ old('presenter_bio') }}</textarea>
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('workshops.index') }}" class="text-sm text-slate-500 hover:text-slate-700">إلغاء
                        والعودة</a>
                    <button
                        class="rounded-2xl bg-indigo-600 px-8 py-3 text-sm font-semibold text-white hover:bg-indigo-700">إرسال
                        للمراجعة</button>
                </div>
            </form>
        @endif
