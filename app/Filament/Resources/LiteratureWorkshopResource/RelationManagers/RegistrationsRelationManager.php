...
namespace App\Filament\Resources\LiteratureWorkshopResource\RelationManagers;
    ...
    public function table(Table $table): Table
    {
        return $table->columns([
            ...
            Tables\Columns\TextColumn::make('created_at')->label('التاريخ')->since(),
                    ...
                    Filter::make('registered_range')->label('تاريخ التسجيل')->form([
                        Forms\Components\DatePicker::make('from')->label('من'),
                        Forms\Components\DatePicker::make('until')->label('إلى'),
                    ])->query(fn($query, array $data) => $query
                            ->when($data['from'] ?? null, fn($q, $date) => $q->whereDate('created_at', '>=', $date))
                            ->when($data['until'] ?? null, fn($q, $date) => $q->whereDate('created_at', '<=', $date))),
                ])->defaultSort('id', 'desc')->headerActions([])->actions([
                    Tables\Actions\ViewAction::make()->modalHeading('تفاصيل التسجيل')
